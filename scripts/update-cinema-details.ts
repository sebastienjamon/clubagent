...
async function updateAgent() {
    console.log("Updating 'Box Office' agent...");

    // 1. Find the agent
    const { data: agents, error: findError } = await supabase
        .from('agents')
        .select('id, name')
        .ilike('name', '%Box Office%')
        .limit(1);
    ...
    const { error: updateError } = await supabase
        ...
        .update({
            name: "Cinema",
            business_name: "The Grand Theater",
            banner_url: "/theater-banner.png"
        })
        .eq('id', agent.id);
    ...
    if (updateError) {
        ...
        console.log("- Name: Cinema");
        console.log("- Business Name: The Grand Theater");
        console.log("- Banner URL: /theater-banner.png");
    }
}
